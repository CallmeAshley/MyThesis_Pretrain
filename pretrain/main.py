...
def main_pt(permutations=None):
    ...
    dataset_train = build_dataset_to_pretrain(args.input_size, permutations=permutations)
    ...
    from encoder import SparseConv2d
    model.module.sparse_encoder.sp_cnn.conv1 = SparseConv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False).to(args.device)
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            ...
            stats = pre_train_one_ep(ep, args, tb_lg, itrt_train, iters_train, model, optimizer, ce_jigsaw)
            last_loss = stats['last_loss']
            min_loss = min(min_loss, last_loss)
            performance_desc = f'{min_loss:.4f} {last_loss:.4f}'
            
            if (ep % 50 == 0) or (ep == args.ep-1):
                misc.save_checkpoint_with_meta_info_and_opt_state(f'{args.model}_withdecoder_epoch{ep}.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
                misc.save_checkpoint_model_weights_only(f'{args.model}_epoch{ep}.pth', args, model_without_ddp.sparse_encoder.sp_cnn.state_dict())
...
def pre_train_one_ep(ep, args: arg_util.Args, tb_lg: misc.TensorboardLogger, itrt_train, iters_train, model: DistributedDataParallel, optimizer, ce_jigsaw):
    ...
    for it, inp in enumerate(me.log_every(iters_train, itrt_train, 3, header)):
        ...
        SparK.forward
        # loss, only_recon, recon_area  = model(inp, active_b1ff=None, vis=False, label=label)
        # loss = model(inp, active_b1ff=None, vis=False, jigsaw_seq=jigsaw_seq)
        loss, pred_jig = model(inp, active_b1ff=None, vis=False, jigsaw_seq=jigsaw_seq)
        
        loss_jig = ce_jigsaw(pred_jig, label)
        loss = a * loss + b * loss_jig
        
        # ssim_value = ssim(recon_area[0,0], only_recon[0,0], data_range=recon_area[0,0].max() - recon_area[0,0].min())
        # ssim_values.append(ssim_value)
        # psnr_value = cv2.PSNR(recon_area[0,0], only_recon[0,0])
        # psnr_values.append(psnr_value)
        
        p += (pred_jig == label).sum().item()
        nomatch += (pred_jig != label).sum().item()
        all += label.size(0)
...
if __name__ == '__main__':
    ...
    permutations = np.load('/mai_nas/LSH/MRSpineSeg_Challenge_SMU/RandomGapNPuzzlueN/permutation_1000.npy').tolist()
    
    main_pt(permutations=permutations)
