...
def sp_conv_forward(self, x: torch.Tensor):
    ...
    x = x * _get_active_ex_or_ii(H=x.shape[2], W=x.shape[3], returning_active_ex=True)    # (BCHW) *= (B1HW), mask the output of conv
    return x
...
def sp_bn_forward(self, x: torch.Tensor):
    ...
    bchw[ii] = nc
    bchw = bchw.permute(0, 3, 1, 2).contiguous()
    return bchw
...
class SparseConvNeXtBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sparse:
            x = x * _get_active_ex_or_ii(H=x.shape[2], W=x.shape[3], returning_active_ex=True)
