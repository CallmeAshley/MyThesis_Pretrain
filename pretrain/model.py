...
class SparK(nn.Module):
    def __init__(
        ...
        # self.jigsaw = torch.nn.Sequential(*[torch.nn.Linear(2048, 1024),
    ...
    def forward_jig(self, img, seq, device):
        
        trans = Resize(spatial_size=(225,225), mode="bicubic")
        trans2 = Resize(spatial_size=(224,224), mode="bicubic")
        
        B,C,H,W = img.shape
        
        temp = torch.zeros((B,C,H,W)).to(device)
        ...
        for k in range(B):
            ...
            for i in range(3):
                for j in range(3):
                    piece = sample[:, i * 75: (i + 1) * 75, j * 75: (j + 1) * 75]   # jigsaw용 img
                    imgclips.append(piece)
            
            imgclips = [imgclips[item] for item in seq[k]]
            imgclips = torch.stack(imgclips, dim=0)
        
            a = torch.concat((imgclips[0], imgclips[1], imgclips[2]), dim=2)
            b = torch.concat((imgclips[3], imgclips[4], imgclips[5]), dim=2)
            c = torch.concat((imgclips[6], imgclips[7], imgclips[8]), dim=2)
        ...
        featuremap = self.sparse_encoder(temp)[3]
        featuremap = torch.mean(featuremap,dim=(-2,-1))
        
        featuremap = self.jigsaw(featuremap)
    ...
    def forward_mae(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False):
        # step1. Mask
        if active_b1ff is None:     # rand mask
            active_b1ff: torch.BoolTensor = self.mask(inp_bchw.shape[0], inp_bchw.device)  # (B, 1, f, f)     #어떤 패치를 가릴 것인지!
        encoder._cur_active = active_b1ff    # (B, 1, f, f)
        active_b1hw = active_b1ff.repeat_interleave(self.downsample_raito, 2).repeat_interleave(self.downsample_raito, 3)  # (B, 1, H, W)        masked_bchw = inp_bchw * active_b1hw  # 마스킹 된 이미지
        masked_bchw = inp_bchw * active_b1hw
        self.masked_bchw = masked_bchw
        # masking_area = 1-(active_b1hw.detach().cpu().numpy())
        # recon_area = inp_bchw.detach().cpu().numpy() * masking_area
        
        # step2. Encode: get hierarchical encoded sparse features (a list containing 4 feature maps at 4 scales)
        fea_bcffs: List[torch.Tensor] = self.sparse_encoder(masked_bchw)    # 4개의 feature map 획득 for 계층적 이용을 위해
        fea_bcffs.reverse()  # after reversion: from the smallest feature map to the largest   #피쳐맵 순서 바꾸기
        
        # step3. Densify: get hierarchical dense features for decoding
        cur_active = active_b1ff     # (B, 1, f, f)  # where to mask
        to_dec = []
        for i, bcff in enumerate(fea_bcffs):  # from the smallest feature map to the largest
            if bcff is not None:
                bcff = self.densify_norms[i](bcff)   #통과한다고 shape이 바뀌진 않음.. BN이니까 당연?
                mask_tokens = self.mask_tokens[i].expand_as(bcff)   # 마스크 토큰을 피쳐맵과 사이즈 맞춰줌..
            ...
            to_dec.append(bcff)   # masking token이 채워지고 채널이 바뀐 피쳐맵
            cur_active = cur_active.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3)  # dilate the mask map, 피쳐맵 사이즈에 맞도록 마스크 사이즈 조절
        
        # step4. Decode and reconstruct
        rec_bchw = self.dense_decoder(to_dec)
        # only_recon = rec_bchw.detach().cpu().numpy() * masking_area
        ...
        recon_loss = l2_loss.mul_(non_active).sum() / (non_active.sum() + 1e-8)  # loss only on masked (non-active) patches, 49개의 패치 중 어떤 패치가 마스킹되었는지 알려주는 게 non_active.
        ...
        if vis:
            ...
            # return recon_loss, rec_or_inp
        else:
            masked_bchw = inp_bchw * active_b1hw
            rec_bchw = self.unpatchify(rec * var + mean)
            rec_or_inp = torch.where(active_b1hw, inp_bchw, rec_bchw)
            # return recon_loss, only_recon, recon_area
            return recon_loss, rec_or_inp, inp_bchw.device
        
    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False, jigsaw_seq=None):
        recon_loss, rec_or_inp, device = self.forward_mae(inp_bchw, active_b1ff, vis)
        pred_jig = self.forward_jig(rec_or_inp, jigsaw_seq, device)
        
        return recon_loss, pred_jig
        # return recon_loss
    
    def patchify(self, bchw):
        p = self.downsample_raito
        h, w = self.fmap_h, self.fmap_w      #  패치 사이즈 32x32  ---> 패치 갯수는 49개
        B, C = bchw.shape[:2]
